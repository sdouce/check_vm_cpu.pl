...
sub main {
	...
	my $np = Nagios::Plugin->new(
		...
		. "    -l <command> [ -s <subcommand> ] [ -T <timeshift> ] [ -i <interval> ] [ -m <vcore> ]\n"
		...
		. "            + maxlimited - CPU maxlimited time in ms\n"
	...
	$np->add_arg(
		spec => 'vcore|m=s',
		help => "-m, --vcore=<vcore number on VM> \n"
		. "   Number of VCPU for accurate calculation o cpu ready or max limited ",
		required => 0,
	);
	...
	eval
	{
		...
		if (defined($vmname))
		{
			if ($command eq "CPU")
			{
				($result, $output) = vm_cpu_info($vmname, $np, local_uc($subcommand),$vcore );
			}
...
sub vm_cpu_info
{
	my ($vmname, $np, $subcommand ,$vcore, ) = @_;
	...
	if (defined($subcommand))
	{
		...
		elsif ($subcommand eq "READY")
		{
			...
			if (defined($values))
			{
				my $value = simplify_number(convert_number($$values[0][0]->value)/(3000*$vcore));
				$np->add_perfdata(label => "cpu_ready", value => $value, uom => '%', threshold => $np->threshold);
				$output = "\"$vmname\" cpu ready=" . $value . " %";
				$res = $np->check_threshold(check => $value);
			}
		}
		elsif ($subcommand eq "MAXLIMITED")
		{
			$values = return_host_vmware_performance_values($vmname, $defperfargs,'cpu', ('maxlimited.summation:'));
			if (defined($values))
			{
				my $value = simplify_number(convert_number($$values[0][0]->value)/(3000*$vcore));
				$np->add_perfdata(label => "max_limited", value => $value, uom => '%', threshold => $np->threshold);
				$output = "\"$vmname\" cpu maxlimited=" . $value . "%";
				$res = $np->check_threshold(check => $value);
			}
		}		
		else
